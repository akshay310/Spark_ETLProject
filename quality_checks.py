...
def quality_checks(df):
    ...
    try:
        ...
        expectations = {
            "trip_distance": ge_df.expect_column_values_to_be_between(
                "trip_distance", min_value=0.05, strict_min=True
                ),
            "fare_amount": ge_df.expect_column_values_to_be_between(
                "fare_amount", min_value=2, strict_min=True
                ),
            "tip_amount": ge_df.expect_column_values_to_be_between(
                "tip_amount", min_value=0, max_value=df.select("fare_amount").first()[0] * 0.5
                ),
            "trip_duration": ge_df.expect_column_values_to_be_between(
                "trip_duration", min_value=30, max_value=14400
                ),
            "rate_code": ge_df.expect_column_values_to_be_in_set(
                "rate_code", [1, 2, 3, 4, 5, 6]
                ),
            "store_and_fwd_flag": ge_df.expect_column_values_to_be_in_set(
                "store_and_fwd_flag", ["Y", "N"]
                ),
            "payment_type": ge_df.expect_column_values_to_be_in_set(
                "payment_type", [1, 2, 3, 4, 6]
                ),
        }
        ...
        validation_status = df.withColumn(
            "validation_status",
             when(col("row_index").isNotNull(), "PASS")
             )

        for col_name, failed_values in failed_conditions:
            validation_status = validation_status.withColumn(
                "validation_status",
                when((col(col_name).isin(failed_values)) |
                (col("validation_status") == "FAIL"), "FAIL").otherwise("PASS")
            )


        # Segregate the dataset into "pass" ancdd "fail"
        good_records = validation_status.filter(
            col("validation_status") == "PASS"
            ).drop("validation_status")
        bad_records = validation_status.filter(
